...
import pl.dszczygiel.jdbc.utils.CQLUtils;

public class CassandraDatabaseMetaData implements DatabaseMetaData{
	ResultMessage columnsResult;
	ResultMessage tablesResult;
	ResultMessage typesResult;
	List<CQLUdtMetadata> udtsMeta;
	
	public CassandraDatabaseMetaData(ResultMessage columnsResult, ResultMessage tablesResult, ResultMessage typesResult) throws CQLException {
		
		this.columnsResult = columnsResult;
		this.tablesResult = tablesResult;
		this.typesResult = typesResult;
		
		getUdtTypes(typesResult);
	}
	
	private void getUdtTypes(ResultMessage types) throws CQLException{
		udtsMeta = new ArrayList<CQLUdtMetadata>();
		for(Row r : types.getRows()) {
			String keyspace = (String) r.getValueByName("keyspace_name", types.getColumnSpecifications());
			String name = (String) r.getValueByName("type_name", types.getColumnSpecifications());
			List<String> names = (List<String>) r.getValueByName("field_names", types.getColumnSpecifications());
			List<String> fieldTypes = (List<String>) r.getValueByName("field_types", types.getColumnSpecifications());
			List<CQLTypeMetadata> typesMeta = new ArrayList<CQLTypeMetadata>();
			for(String s : fieldTypes) {
				typesMeta.add(CQLUtils.getTypeMetadataFromText(s));
			}
			CQLUdtMetadata meta = new CQLUdtMetadata(CQLType.UDT, keyspace, name, names, typesMeta);
			udtsMeta.add(meta);
		}
	}
	
	public UserDefinedType getUdtByName(String keyspace, String name) throws CQLException {
		for(CQLUdtMetadata meta : udtsMeta) {
			if(meta.getKeyspace().equalsIgnoreCase(keyspace) && meta.getName().equalsIgnoreCase(name))
				return new UserDefinedType(meta);
		}
		throw new CQLException("UDT " + name + " doesn't exist in keyspace " + keyspace);
	}
